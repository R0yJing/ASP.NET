...
namespace MelanomaClassification.Services
{
    public class DatabaseService
    {
        ...
        internal static Task<string> GetUserPreferredClassifier()
        {
        ...
        public static void WaitUntilFinishedInit()
        {
            while (!finishedInit)
            {
        ...
        public static async Task<bool> CheckIfExisted()
        {
            ...
            try
            {
                ...
                prefTblExists = await DbConn.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND" +
                    " name='prefTbl'");
                ...
                if (prefTblExists != 1)
                {
                    await DbConn.CreateTableAsync<SQL_ModelPreferences>();

                    prefTblExists = await DbConn.ExecuteScalarAsync<int>
                    ("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='prefTbl'");
            ...
            if (imageTblExists + predictTblExists + predictTblExists < 3)
            {
        ...
        public static async Task<string> GetUserPrefOfAsync(string attr)
        {
            var attrValue = await DbConn.ExecuteScalarAsync<string>("SELECT ? FROM prefTbl WHERE Username='?'", 
                new string[] { attr, ModelAccountPage.Username });
            return attrValue;
        }
        

        public static async void DeleteByIdAsync(string id)
        {
        ...
        public static async Task PutAsync(ModelPredictionWrapper predictWrapper)
        {
            ...
            try
            {
                ...
                var predList = new List<SQL_ModelPrediction>();
                for (int i=0; i< 3; i++)
                {
                    var prediction = new SQL_ModelPrediction
                    {
                        Prob = predictWrapper.Predictions[i].Probability.ToString(),
                        Id = index,
                        Tag = predictWrapper.Predictions[i].TagName
                    };
        ...
        public static async Task<int> DropAllTables()
        {
            ...
            int n2 = await DbConn.DropTableAsync<SQL_ModelPreferences>();
        ...
        public static async Task<bool> DeleteCurrentUser()
        {
            ...
            Task<int> deletedPref = DbConn.ExecuteAsync("DELETE FROM prefTbl WHERE Username='?'", new string[] { ModelAccountPage.Username });

            Task.WaitAll(deletedImages, deletedPredicts, deletedPref);
